...
legacyJSON.apps.forEach(app => {
  ...
  const environmentSource =
      path.relative(path.join(__dirname), path.join(__dirname, app.root, app.environmentSource));
      ...
      configurations[(env === 'prod' ? 'production' : env)] = {
        'fileReplacements': [{
          ...
          'replaceWith': path.relative(
              path.join(__dirname), path.join(__dirname, app.root, app.environments[env]))
        }],
        'optimization': true,
        'outputHashing': 'all',
        'sourceMap': false,
  ...
  if (app.stylePreprocessorOptions) {
    ...
    options.stylePreprocessorOptions.includePaths =
        options.stylePreprocessorOptions.includePaths.map(stylePath => {
  ...
  if (platformType === 'browser') {
    options['index'] =
        path.relative(path.join(__dirname), path.join(__dirname, app.root, app.index));
    options['assets'] = [];
    options['styles'] = [];
    options['scripts'] = [];
    ...
    app.scripts.forEach(script => {
      if (typeof script === 'string') {
        const filePath =
            path.relative(path.join(__dirname), path.join(__dirname, app.root, script));
  ...
  if (app.polyfills) {  // polyfills dose not exists on Server cli setting
    options['polyfills'] =
        path.relative(path.join(__dirname), path.join(__dirname, app.root, app.polyfills));
  }
    ...
    if (app.testTsconfig) {
      const testOptions = {
        ...
        'polyfills':
            path.relative(path.join(__dirname), path.join(__dirname, app.root, app.polyfills)),
        'tsConfig':
            path.relative(path.join(__dirname), path.join(__dirname, app.root, app.testTsconfig)),
      ...
      testOptions['tsConfig'] =
          path.relative(path.join(__dirname), path.join(__dirname, app.root, app.testTsconfig));
      Object.assign(
          newJSON.projects[app.name].architect,
          {'test': {'builder': '@angular-devkit/build-angular:karma', 'options': testOptions}});
    }
    Object.assign(newJSON.projects[app.name].architect, {
      'serve': {
        'builder': '@angular-devkit/build-angular:dev-server',
        'options': {'browserTarget': app.name + ':build'},
        'configurations': {'production': {'browserTarget': app.name + ':build:production'}}
      },
      'extract-i18n': {
