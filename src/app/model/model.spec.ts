...
import {BallActions, DrillsState, EntityType, PlayerActions} from './types';
...
fdescribe('animations', () => {
  ...
  it('should return start position for no actions at the start', () => {
    expect(
        positionForKeyframeHelper(
            {id: 1, type: EntityType.PLAYER, icon: 'player_blue', start: {posX: 1, posY: 1}}, 1, {
              ...initialState,
            }))
  ...
  it('should return start position for no actions in the future', () => {
    expect(
        positionForKeyframeHelper(
            {id: 1, type: EntityType.PLAYER, icon: 'player_blue', start: {posX: 1, posY: 1}}, 100, {
              ...initialState,
            }))
        .toEqual({posX: 1, posY: 1});
  });
  ...
  it('should return position for one action', () => {
    const player = {id: 1, type: EntityType.PLAYER, icon: 'player_blue', start: {posX: 1, posY: 1}};
  ...
  it('should return position for ball that starts in possession of player', () => {
    const player = {id: 1, type: EntityType.PLAYER, icon: 'player_blue', start: {posX: 5, posY: 5}};
    const volleyball = {id: 2, type: EntityType.VOLLEYBALL, icon: 'volleyball', start: {posX: 1, posY: 1}};
    const state: DrillsState = {
      ...initialState,
      animations: [{
        ...initialState.animations[0],
  ...
  it('should return position for one action with possession', () => {
    ...
    expect(positionForKeyframeHelper(volleyball, 100, state)).toEqual({posX: 5, posY: 5});
  });
