...
export class ActionComponent implements OnInit {
  ...
  readonly jumping: Observable<boolean>;
  ...
  constructor(private readonly store: Store<{}>) {
    ...
    this.jumping = this.currentAction.pipe((map((action) => {
      if (!action) {
        return false;
      }
  ...
  onLengthChange(start: number, end: number): void { console.warn('not hooked up'); }
  ...
  onJumpChange(val: boolean) {
    const id = this.getActionId();
    if (id == null) {
      throw new Error('No action selected during type change');
    }
    this.store.dispatch(new ChangeActionJump(id, val));
  }
  ...
  rotate(degrees: number) {
    ...
    this.store.dispatch(new Rotate(id, degrees));
  }
